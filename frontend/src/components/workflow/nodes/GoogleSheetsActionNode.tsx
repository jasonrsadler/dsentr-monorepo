...
interface GoogleSheetsActionNodeProps {
  id: string
  selected: boolean
...
type GoogleSheetsActionNodeRenderProps =
  BaseActionNodeChildrenProps<ActionNodeData>

type GoogleSheetsActionNodeContentProps = {
  id: string
  planTier?: string | null
  onRestrictionNotice?: (message: string) => void
  baseProps: GoogleSheetsActionNodeRenderProps
}

export default function GoogleSheetsActionNode({
  id,
  selected,
  planTier,
  onRestrictionNotice,
  ...
  canEdit = true,
  runAvailability
}: GoogleSheetsActionNodeProps) {
  return (
    <BaseActionNode<ActionNodeData>
      id={id}
      selected={selected}
      canEdit={canEdit}
      fallbackLabel="Google Sheets action"
      defaultExpanded
      onRun={onRun}
      isRunning={isRunning}
...
function GoogleSheetsActionNodeContent({
  id,
  planTier,
  onRestrictionNotice,
  baseProps
}: GoogleSheetsActionNodeContentProps) {
  const {
    selected,
    runState,
    nodeData,
        ...
        <ActionNodeSummary
          ...
          onPlanUpgrade={
            controller.planRestrictionMessage
              ? controller.handlePlanUpgradeClick
              : undefined
          }
          hint="Open the Sheets flyout to pick a spreadsheet, worksheet, and columns."
        />
      </div>
