...
interface NotionActionNodeProps {
  id: string
  selected: boolean
...
type NotionActionNodeRenderProps = BaseActionNodeChildrenProps<ActionNodeData>

type NotionActionNodeContentProps = {
  id: string
  planTier?: string | null
  onRestrictionNotice?: (message: string) => void
  baseProps: NotionActionNodeRenderProps
}

export default function NotionActionNode({
  id,
  selected,
  planTier,
  onRestrictionNotice,
  ...
  canEdit = true,
  runAvailability
}: NotionActionNodeProps) {
  return (
    <BaseActionNode<ActionNodeData>
      id={id}
      selected={selected}
      canEdit={canEdit}
      fallbackLabel="Notion action"
      defaultExpanded
      onRun={onRun}
      isRunning={isRunning}
...
function NotionActionNodeContent({
  id,
  planTier,
  onRestrictionNotice,
  baseProps
}: NotionActionNodeContentProps) {
  const {
    selected,
    runState,
    nodeData,
    ...
    >
      ...
      <div className="p-3">
        ...
        <ActionNodeSummary
          ...
          hint="Open the Notion flyout to choose a connection, database, and fields."
        />
      </div>
      <AnimatePresence>
