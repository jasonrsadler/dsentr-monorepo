...
export default function WebhookAction({
  ...
  onChange?: (args: Partial<WebhookActionProps>, nodeHasErrors: boolean, childDirty: boolean) => void;
}) {
  ...
  const hasErrors = (p: Partial<WebhookActionProps>) => {
    ...
    if (["POST", "PUT", "PATCH"].includes(p.method || "")) {
      if (p.bodyType === "raw" && !p.body?.trim()) errors.body = "Request body is required";
      if (p.bodyType === "json") {
        try { JSON.parse(p.body || ""); }
        catch { errors.body = "Invalid JSON"; }
      }
      if (p.bodyType === "form" && (!p.formBody || p.formBody.length === 0)) errors.body = "Form body cannot be empty";
    }

    // Auth validation
    if (p.authType === "basic") {
      if (!p.authUsername?.trim()) errors.authUsername = "Username required for Basic Auth";
      if (!p.authPassword?.trim()) errors.authPassword = "Password required for Basic Auth";
    }
    if (p.authType === "bearer" && !p.authToken?.trim()) errors.authToken = "Token required for Bearer Auth";
    ...
    <div className="flex flex-col gap-2">
      ...
      <KeyValuePair
        ...
        onChange={(updatedVars, nodeHasErrors, childDirty) => {
          ...
          onChange?.({ ...params, headers: updatedVars }, nodeHasErrors, childDirty);
        }}
      />
      ...
      <KeyValuePair
        ...
        onChange={(updatedVars, nodeHasErrors, childDirty) => {
          ...
          setDirty(prev => prev || childDirty);
          onChange?.({ ...params, queryParams: updatedVars }, nodeHasErrors, childDirty);
        }}
      />
      ...
      {["raw", "json"].includes(params.bodyType || "") && (
        <NodeTextAreaField
          placeholder={params.bodyType === "raw" ? "Request Body" : 'JSON Body (e.g. {"key":"value"})'}
          value={params.body || ""}
          rows={6}
      ...
      {params.bodyType === "form" && (
        <KeyValuePair
          ...
          onChange={(updatedVars, nodeHasErrors, childDirty) => {
            ...
            setDirty(prev => prev || childDirty);
            onChange?.({ ...params, formBody: updatedVars }, nodeHasErrors, childDirty);
          }}
        />
      )}
      ...
      {params.authType === "basic" && (
        <>
          ...
          />
          {webhookErrors.authUsername && <p className={errorClass}>{webhookErrors.authUsername}</p>}
          <NodeInputField
          ...
          />
          {webhookErrors.authPassword && <p className={errorClass}>{webhookErrors.authPassword}</p>}
        </>
      )}
      ...
      {params.authType === "bearer" && (
        <>
          ...
          />
          {webhookErrors.authToken && <p className={errorClass}>{webhookErrors.authToken}</p>}
        </>
      )}
    </div>
  );
