...
export default function MailGunAction({ args, onChange }: { args: MailGunActionProps, onChange?: (args: Partial<MailGunActionProps>, hasErrors: boolean, dirty: boolean) => void }) {
  ...
  return (
    <>
      <div className="space-y-2">
        ...
        />
        {mailGunErrors.domain && <p className={errorClass}>{mailGunErrors.domain}</p>}
        ...
        <NodeInputField
          ...
          type="password"
        />
        {mailGunErrors.apiKey && <p className={errorClass}>{mailGunErrors.apiKey}</p>}

        <MailgunRegionDropdown
          value={params.region || ""}
          onChange={(val: string) => updateField("region", val)}
        />
        {mailGunErrors.region && <p className={errorClass}>{mailGunErrors.region}</p>}
        <NodeInputField
          type="email"
          placeholder="From"
          value={params.from || ""}
        ...
        />
        {mailGunErrors.from && <p className={errorClass}>{mailGunErrors.from}</p>}
        ...
        {params.template?.trim() && (
          <KeyValuePair
            ...
            onChange={(updatedVars, nodeHasErrors, childDirty) => {
              ...
              onChange?.({ ...params, variables: updatedVars }, nodeHasErrors, childDirty)
            }}
          />
        )}
        {!params.template && (
          <>
            ...
            />
            {mailGunErrors.subject && <p className={errorClass}>{mailGunErrors.subject}</p>}
            <NodeTextAreaField
            ...
            />
            {mailGunErrors.body && <p className={errorClass}>{mailGunErrors.body}</p>}
          </>
        )}
      </div>
    </>
