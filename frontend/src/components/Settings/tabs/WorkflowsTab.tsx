...
import { deleteWorkflow, listWorkflows, WorkflowRecord } from '@/lib/workflowApi'
...
export default function WorkflowsTab({ workflows: propWorkflows, onDeleted }: Props) {
  const [items, setItems] = useState<WorkflowRecord[]>(propWorkflows ?? [])
  useEffect(() => { setItems(propWorkflows ?? []) }, [propWorkflows])
  useEffect(() => {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setConfirmText('')
      try { window.dispatchEvent(new CustomEvent('workflow-deleted', { detail: { id: selected.id } })) } catch {}
    } finally {
      setBusy(false)
      setConfirming(false)
    }
      ...
      {confirming && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-xl">
          <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-xl w-80 border border-zinc-200 dark:border-zinc-700">
            ...
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mb-4">This action cannot be undone.</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirming(false)} className="px-3 py-1 text-xs rounded border">Cancel</button>
              <button onClick={handleDelete} disabled={busy} className="px-3 py-1 text-xs rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-50">
