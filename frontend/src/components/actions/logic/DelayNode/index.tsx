...
export default function DelayNodeConfig({
  ...
  const updateWaitUntil = useCallback(
    (dateStr: string, hour: number, minute: number, second: number) => {
      const iso = buildIso(dateStr, hour, minute, second)
      emitConfig({
        ...normalizedConfig,
        wait_until: iso
      })
...
function DateTimeFields({
  ...
  waitUntil: string
  onChange: (date: string, hour: number, minute: number, second: number) => void
}) {
    ...
    <div className="space-y-2">
      ...
      <div className="sr-only">
        ...
        <input
          ...
          value={parts.date}
          onChange={(e) =>
            onChange(e.target.value, parts.hour, parts.minute, parts.second)
          }
        />
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="relative">
          <button
            ...
            className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-left text-sm font-medium text-zinc-900 shadow-sm transition hover:border-blue-400 hover:shadow focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40 dark:border-zinc-700 dark:bg-zinc-900/70 dark:text-zinc-100"
          >
            {parts.date || 'Select date (UTC)'}
          </button>
          <AnimatePresence>
            {calendarOpen ? (
              <motion.div
              ...
              >
                <ScheduleCalendar
                  ...
                  onSelectDate={(isoDate) => {
                    ...
                    onChange(nextDate, parts.hour, parts.minute, parts.second)
                  }}
                />
              </motion.div>
            ) : null}
              ...
              >
                <ScheduleTimePicker
                  ...
                  onSelect={(time) => {
                    ...
                    onChange(parts.date, hours, minutes, seconds)
                  }}
