...
- Workflow canvas flyout: added a right-side flyout that mirrors the selected node's controls inside a dedicated React Flow instance so users can edit node details without interacting with the canvas element directly.
