  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950 text-slate-100">
        <div className="animate-pulse text-sm text-slate-400">Checking session...</div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 text-slate-100">
        <div className="card max-w-md text-center text-sm">
          <div className="mb-2 text-lg font-semibold text-red-300">Not an admin</div>
          <p className="text-slate-400">
            {error ?? 'Your account is signed in but does not have admin access.'}
          </p>
