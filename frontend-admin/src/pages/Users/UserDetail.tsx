...
import { getUser, getUserConnections, getUserWorkspaces } from '../../api/users';
import {
  AdminUserDetail,
  ConnectionSummary,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-100">{user?.email ?? id}</h2>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="card">
          <div className="mb-2 text-sm font-semibold text-slate-200">Workspaces</div>
          <Table
            data={workspaces}
            columns={[
              {
                ...
                header: 'Workspace',
                render: (row) => (
                  <Link className="text-accent" to={`/workspaces/${row.workspace.id}`}>
                    {row.workspace.name}
                  </Link>
                ),
              },
              { key: 'role', header: 'Role' },
              { key: 'plan', header: 'Plan', render: (row) => row.workspace.plan },
            ]}
            empty="No workspaces"
          />
        </div>
        <div className="card">
          <div className="mb-2 text-sm font-semibold text-slate-200">OAuth connections</div>
          <Table
            data={connections}
            columns={[
