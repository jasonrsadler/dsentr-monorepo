...
export default function WorkspacesList() {
  ...
  return (
    <div className="space-y-4">
      ...
      {view === 'chart' ? (
        <ChartView title="Runs per workspace (current page)" data={chartData} type="bar" />
      ) : (
        <>
          <Table
            data={workspaces}
            columns={[
              { key: 'id', header: 'ID', render: (row) => (row as WorkspaceSummary).id.slice(0, 8) },
              {
                key: 'name',
                header: 'Name',
                render: (row) => (
                  <Link className="text-accent" to={`/workspaces/${(row as WorkspaceSummary).id}`}>
              ...
              { key: 'run_count', header: 'Runs' },
              {
                key: 'created_at',
                header: 'Created',
                render: (row) => new Date((row as WorkspaceSummary).created_at).toLocaleDateString(),
              },
            ]}
