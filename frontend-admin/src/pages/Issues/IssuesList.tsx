  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-wide text-slate-400">Issues</div>
          ...
          <Table
            ...
            columns={[
              ...
              { key: 'status', header: 'Status' },
              {
                key: 'created_at',
                header: 'Created',
                render: (row) => new Date((row as IssueSummary).created_at).toLocaleString(),
              },
              {
                key: 'view',
                header: '',
                render: (row) => (
                  <Link className="text-accent" to={`/issues/${(row as IssueSummary).id}`}>
                    View
                  </Link>
                ),
              },
