...
export default function SMTPAction({ args, onChange }: { args: SMTPActionProps, onChange?: (args: Partial<SMTPActionProps>, hasErrors: boolean, dirty: boolean) => void }) {
  ...
  const hasErrors = (updatedParams: Partial<SMTPActionProps>) => {
    ...
    if (!updatedParams.smtpHost?.trim()) errors.smtpHost = "SMTP Host is required"
    const port = Number(updatedParams.smtpPort)
    if (!updatedParams.smtpPort || Number.isNaN(port) || port <= 0) errors.smtpPort = "Valid SMTP Port is required"
    if (!updatedParams.smtpUser?.trim()) errors.smtpUser = "SMTP User is required"
    if (!updatedParams.smtpPassword?.trim()) errors.smtpPassword = "SMTP Password is required"
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      />
      {smtpErrors.smtpHost && <p className={errorClass}>{smtpErrors.smtpHost}</p>}
      <NodeInputField
      ...
      />
      {smtpErrors.smtpPort && <p className={errorClass}>{smtpErrors.smtpPort}</p>}
      <NodeInputField
        placeholder="Username"
        value={params.smtpUser || ""}
        onChange={val => updateField("smtpUser", val)}
      />
      {smtpErrors.smtpUser && <p className={errorClass}>{smtpErrors.smtpUser}</p>}
      <NodeInputField
        type="password"
      ...
      />
      {smtpErrors.smtpPassword && <p className={errorClass}>{smtpErrors.smtpPassword}</p>}
      <NodeCheckBoxField
        ...
        onChange={val => {
          ...
          setParams(prev => ({
            ...prev,
            smtpTls: checked,
            smtpPort: prev.smtpPort === 25 || prev.smtpPort === 587
              ? (checked ? 587 : 25)
              : prev.smtpPort
          }))
          setDirty(true)
